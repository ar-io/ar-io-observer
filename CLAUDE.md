...
2. **System Bootstrap** (`src/system.ts`): Dependency injection container
   ...
   - Constructs the Solana RPC + signer pair and the two
     `SolanaARIOWriteable` instances (operator-signed cranker contract +
     observer-signed `save_observations` contract)
...
- **Name Sources**: `SolanaNamesSource` resolves prescribed names via the
  SDK (`getPrescribedNames` walks epoch hash → ArnsRecord PDA) and
  surfaces the registry as `ArnsNameList` for random-name sampling.
...
- **Epoch Sources**: `SolanaEpochSource` reads the current Epoch PDA via
  ...
  sentinel — Solana epochs are not Arweave-block-aligned.
...
- `OBSERVER_WALLET`: Display label for the `/info` endpoint. Actual
  protocol identities come from `SOLANA_KEYPAIR_PATH` /
  `OBSERVER_KEYPAIR_PATH` / etc.
...
### Observation submission (`save_observations`)

When `SUBMIT_CONTRACT_INTERACTIONS=true`,
...
- The on-chain `Observation.report_tx_id` field stores the **raw 32-byte
  ...
  permaweb audits.
- `getFailedGatewaySummaryFromReport`
  (`src/store/failed-gateway-summary.ts`) extracts the failed-gateway
  pubkey list; the SDK turns it into a 375-byte bitmap matching the
  registry order.
